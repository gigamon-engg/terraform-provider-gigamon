...
This folder contains Terraform provider acceptance tests for app resources and traffic map condition coverage.
...
Runs all app acceptance tests (5gcloud + app viz + phase1 app table tests):

```bash
go test ./examples/testing/testacc -v -run 'TestAcc(App5GCloudResource|AppVizResource|Phase1Apps)_'
```
...
```bash
go test ./examples/testing/testacc -v -run 'TestAcc(App5GCloudResource_RequiredFieldMissing|AppVizResource_RequiredFieldMissing|Phase1Apps_RequiredMonitoringSessionID|TrafficMapPhase2Conditions_Validation)$'
```
